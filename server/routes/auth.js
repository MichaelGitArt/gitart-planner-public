...
router.post('/profile/:slug/update', auth({ required: true }), checkSlug, [
	check('name').isLength({ min: 3, max: 30 }).withMessage('Від 3 до 30 символів').matches(/^[a-z0-9а-я ]+$/i).withMessage('Лише букви, цифри та пробіли')
], catchErrors(authController.updateProfile));

router.post('/checkFreeSlug',
	auth({ required: true }),
	checkSlug,
